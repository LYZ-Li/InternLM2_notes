...
- 指令跟随微调
  ...
  - 训练数据：高质量的对话、问答数据
  > 需要高质量的标准——也就是“一问一答”的形式  

![](images/image_04_01.png)
> 增量预训练微调可以看作是知识的不断积累，而指令跟随微调更注重于模型的实际应用能力。在实际项目中，两者可以结合使用，以达到最佳效果。

### 1.2 一条数据的一生
- 原始数据
- 标准格式数据
  - 系统、用户、助手三种角色
- 添加对话模板：这部分XTunner已经封装
- Tokenized数据
- 添加Label
- 开始训练

### 1.3 LoRA 和QLoRA
<img src="images/image_04_02.png" width="300" >

**LoRA:** LOW-RANK ADAPTATION OF LARGE LANGUAGE MODELS  
**Adapter：** LoRA 通过在原本的 Linear 旁，新增一个支路，包含两个连续的小 Linear，新增的这个支路通常叫做 Adapter
> Adapter的参数两远小于原本的Linear，能大幅降低显存消耗

![](images/image_04_03.png)
...
> XTuner 的广泛适配性极大地方便了开发者，无论是从硬件要求还是生态兼容性方面，都展现了其强大的灵活性和实用性。
...
> XTuner 的一键训练功能让微调变得更加简便，对于初学者尤其友好，减少了繁琐的配置步骤。

## 3. 8GB 显存玩转 LLM
### 3.1 优化技巧
- Flash Attention：监视显存占用，自动开启
- DeepSpeed ZeRO

> 通过这些优化技巧，即使在显存有限的情况下，也能高效地进行大模型的训练和推理，这无疑为更多研究者和开发者打开了大模型应用的大门。

## 4. InternLM2 1.8B 模型
### 4.1 模型版本
- InternLM2-1.8B: 基础模型
- InternLM2-Chat-1.8B-SFT: 监督微调对话模型
- InternLM2-Chat-1.8B: 在线 RLHF 模型

> 在 FP16 精度模式下，InternLM2-1.8B 仅需 **4GB** 显存的笔记本显卡即可顺畅运行进行**推理**。拥有 **8GB** 显存的消费级显卡，即可轻松进行 1.8B 模型的**微调**工作。如此低的硬件门槛，非常适合初学者使用，以深入了解和掌握大模型的全链路。

## 5. 多模态 LLM 微调
### 5.1 原理简介
- 文本+图像多模态
![](images/image_04_04.png)
> 训练多模态的过程就是训练Image Projector的过程
### 5.2 LLaVA 方案简介
- 训练阶段：这部分给出海量的数据（图片+图片内容描述）
- 微调阶段：高质量的对话

![](images/image_04_05.png)
...
打开该 python 文件后将[generate_data.py](codes_04generate_data.py)下面的内容复制进去。
...
我是小菜鸡的小助手，内在是上海AI实验室书生·浦语的1.8B大模型哦<|im_end|>
```
通过两者的对比我们其实就可以很清楚的看到，在300轮的时候模型已经学会了在我问 “你是谁” 或者说 “请你介绍一下我自己” 的时候回答 “我是小菜鸡的小助手，内在是上海AI实验室书生·浦语的1.8B大模型哦”。

但是两者的不同是在询问 “你是我的小助手” 的这个问题上，300轮的时候是回答正确的，回答了 “是” ，但是在600轮的时候回答的还是 “我是小菜鸡的小助手，内在是上海AI实验室书生·浦语的1.8B大模型哦” 这一段话。这表明模型在第一批次第600轮的时候已经出现严重的过拟合（即模型丢失了基础的能力，只会成为某一句话的复读机）现象了，到后面的话无论我们再问什么，得到的结果也就只能是回答这一句话了，模型已经不会再说别的话了。因此假如以通用能力的角度选择最合适的权重文件的话我们可能会选择前面的权重文件进行后续的模型转化及整合工作。
